...
public class ColorAssociation extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_color_association);
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        getMenuInflater().inflate(R.menu.color_association, menu);
        return true;
    }
    ...
    public void setPicture(View view) throws IOException{
        ...
        int colorblindness = preferences.getInt("colorblindness",-1);
        boolean colorblind = preferences.getBoolean("colorblind",false);
        InputStream istr;
        ...
        try {
            if(colorblind) {
                if (colorblindness == 0) {
                    filelist = assetManager.list("deuteranopia");
                    ...
                    istr = assetManager.open("deuteranopia/" + filelist[randomImg]);
                    Bitmap bitmap = BitmapFactory.decodeStream(istr);
                    imageView.setImageBitmap(bitmap);
                    istr.close();
                } else if (colorblindness == 1) {
                    filelist = assetManager.list("protanopia");
                    int randomImg = (int) (Math.random() * filelist.length);
                    int spaceIndex = filelist[randomImg].indexOf(".");
                    correctAnswer = filelist[randomImg].substring(0, spaceIndex).trim();
                    istr = assetManager.open("protanopia/" + filelist[randomImg]);
                    Bitmap bitmap = BitmapFactory.decodeStream(istr);
                    imageView.setImageBitmap(bitmap);
                    istr.close();
                } else if (colorblindness == 2) {
                    filelist = assetManager.list("tritanopia");
                    int randomImg = (int) (Math.random() * filelist.length);
                    int spaceIndex = filelist[randomImg].indexOf(".");
                    correctAnswer = filelist[randomImg].substring(0, spaceIndex).trim();
                    istr = assetManager.open("tritanopia/" + filelist[randomImg]);
                    Bitmap bitmap = BitmapFactory.decodeStream(istr);
                    imageView.setImageBitmap(bitmap);
                    istr.close();
                }
            }else{
                filelist = assetManager.list("normalPeopleColors");
                int randomImg = (int) (Math.random() * filelist.length);
                int spaceIndex = filelist[randomImg].indexOf(".");
                correctAnswer = filelist[randomImg].substring(0, spaceIndex).trim();
                istr = assetManager.open("normalPeopleColors/" + filelist[randomImg]);
                Bitmap bitmap = BitmapFactory.decodeStream(istr);
                imageView.setImageBitmap(bitmap);
                istr.close();
            }
    ...
    private class EndGame extends AsyncTask<String, Void, Integer>
    {
        ...
        EndGame()
        {
            SharedPreferences preferences = PreferenceManager.getDefaultSharedPreferences(ColorAssociation.this);
        ...
        protected void onPostExecute(Integer highScore)
        {
            music.release();
            sound.release();
            Intent intent = new Intent(ColorAssociation.this, GameResults.class);
        ...
        protected Integer doInBackground(String... args)
        {
            ...
            if(this.username != null && !this.username.equals("")){
                ...
                {
                    String score = String.valueOf(ColorAssociation.this.score);
    ...
    private class SoundEffects extends AsyncTask<Void, Void, Void>
    {
        public SoundEffects(boolean success) {
            super();
            if(success){
                sound = MediaPlayer.create(ColorAssociation.this, R.raw.success);
            }
            else {
                sound = MediaPlayer.create(ColorAssociation.this, R.raw.fail);
            }
